...
st.set_page_config(page_title="CRISTIANO RONALDO - EDA", layout='wide')

st.sidebar.title("📊 Cristiano Ronaldo Analytics")
...
if selections == "Introduction":
    st.title("⚽ CR7 _EDA")
    st.subheader("CR-All Club Goals Statistics")
    ...
    **Cristiano Ronaldo dos Santos Aveiro** is a Portuguese professional footballer who plays as a forward for Al-Nassr and captains the Portugal national team.
    ...
    for line in lines:
        if line.startswith(' '):
            ...
            data_info.append([parts[0], parts[1], parts[2], parts[3], parts[4]])
    ...
    st.plotly_chart(fig)
    st.write('### Competition Goal Count')
